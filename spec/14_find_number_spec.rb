...
describe FindNumber do
  ...
  let(:random_six) { double('random_number', value: 6) }
    context 'when guess and random_number are equal' do
      subject(:make_guess_six) { described_class.new(0, 9, random_six, 6) }
      ...
      it 'is game over' do
        expect(make_guess_six).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_wrong_guess) { described_class.new(0,9, random_six, 7) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:correct_guess_game) { described_class.new(5,8,number_range, 7)}
      it 'updates min to the same value as max' do
        correct_guess_game.update_range
        expect(correct_guess_game.min).to eq(correct_guess_game.max)
      end

      it 'does not update max' do
        correct_guess_game.update_range
        expect(correct_guess_game.max).to eq(8)
      end
    end

    context 'when guess is 9, min is 8, max is 10' do
      subject(:correct_guess_min) { described_class.new(8, 10, number_range, 9) }
       
      it 'updates max to the same value as min' do 
       correct_guess_min.update_range
       expect(correct_guess_min.max).to eq(correct_guess_min.min)    
      end

      it 'does not update min' do
        correct_guess_min.update_range
        expect(correct_guess_min.min).to eq(8)
      end
    end

    context 'when guess is the same as answer' do
      subject(:correct_first_guess) { described_class.new(4, 9, number_range, 8) }

      it 'does not update max' do
        correct_first_guess.update_range
        expect(correct_first_guess.max).to eq(9)
      end

      
      it 'does not update min' do
        correct_first_guess.update_range
        expect(correct_first_guess.min).to eq(4)
      end
    end

    context 'when guess is nil' do
     subject(:game_guess_nil) { described_class.new(2, 9, number_range) }

      it 'does not update min' do
       game_guess_nil.update_range
       expect(game_guess_nil.min).to eq(2) 
      end

      it 'does not update max' do
        game_guess_nil.update_range
        expect(game_guess_nil.max).to eq(9)
      end
    end
