...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:lose_game) { described_class.new(7, '3') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(lose_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        wrong_input = '15'
        verified_input = game_check.verify_input(wrong_input)
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_wrong_value = 'eyyy'
        second_wrong_value = 'okayyy'
        valid_input = '5'
        allow(game_loop).to receive(:player_input).and_return(first_wrong_value, second_wrong_value, valid_input)
      end
