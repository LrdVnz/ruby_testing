...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:beverage) { described_class.new('mojito') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
       expect(beverage.type).to be('mojito')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
       expect(beverage.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
         subject(:ultimate_drink) { described_class.new('caprisun', 22)}
      # remove the 'x' before running this test
      it 'is full' do
        expect(ultimate_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      # beverage type. In addition, specify ounces to be any number under 16.
        subject(:thadrink)  { described_class.new('milk', 10)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(thadrink).not_to be_full
      end
    end
  end
end
